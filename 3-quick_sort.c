...
/**
 * lomuto_partition - Orders a subset of an array of integers using the Lomuto partition scheme.
 *
 * This function orders a specified subset of an array of integers according to
 * the Lomuto partition scheme with the last element as the pivot. It rearranges
 * the elements in the given range [left, right] so that elements less than or equal
 * to the pivot are on the left, and elements greater than the pivot are on the right.
 *
...
/**
 * lomuto_sort - Implement the quicksort algorithm with Lomuto partition scheme.
 *
 * This function recursively sorts a partition of an array of integers
 * in ascending order using the quicksort algorithm with the Lomuto partition scheme.
 * It operates on the specified range of indices from 'left' to 'right' (inclusive),
 * effectively sorting the elements within that range.
 *
